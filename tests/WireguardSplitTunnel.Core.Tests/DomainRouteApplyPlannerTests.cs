...
public sealed class DomainRouteApplyPlannerTests
{
    ...
    [Fact]
    public void Build_WithExactInterface_PersistsRouteOwnership()
    {
        var resolved = new[]
        {
            new ResolvedRule(new DomainRule("chatgpt.com"), ["104.18.32.47"])
        };

        var plan = DomainRouteApplyPlanner.Build([], resolved, "utun4");

        plan.Snapshot.Should().Equal(
            new ManagedRouteEntry("chatgpt.com", "104.18.32.47", "utun4"));
    }
}
